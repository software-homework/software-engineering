...
public class LoginFrame extends JFrame implements ActionListener
{
	...
	public LoginFrame()
	{
	    ...
	    //-------------------------------------------------------------------------
	    String[] s1 = {"管理员", "库存管理人员", "进货销售人员", "财务人员", "总经理"};
	    ...
	    permissionBC.setLocation(380, 4 * (heightLabel + 10) + 100);
	    permissionBC.setFont(font);
		permissionBC.setEnabled(false);
	    this.add(permissionBC);
	    //-------------------------------------------------------------------------
	    login = new JButton("登录");login.setFont(font);
	    login.setBackground(Color.CYAN);
	    login.setSize(widthLabel, heightLabel);
	    login.setLocation(280, 5 * (heightLabel + 10) + 120);
	    this.add(login);
	    
	    cancel = new JButton("取消");cancel.setFont(font);
	...
	public void actionPerformed(ActionEvent arg0) 
	{
		if(arg0.getSource() == login)
		{
			...
			{
				UserVO userVO = new UserVO(((String)typeBC.getSelectedItem()), 
						nameTF.getText().trim(),
						String.copyValueOf(jPasswordField.getPassword()), 
						((String)permissionBC.getSelectedItem()));
				boolean isSuccess = ubs.login(userVO);
				
				if(isSuccess)
				{
					if(((String)typeBC.getSelectedItem()).equals("库存管理人员"))
					{
						new StockMain(userVO);
					}
					else if(((String)typeBC.getSelectedItem()).equals("进货销售人员"))
					{
		...
		else if(arg0.getSource() == cancel)
		{
			...
			if(((String)typeBC.getSelectedItem()).equals("管理员"))
