...
public class SalesP2 extends JPanel{
	...
	Object[][] newdata;
	//销售单
	public SalesP2(Object[][] cdata,AddSalesFrame asf){
		...
		String[] columnTitle={"数量(可修改)","编号","名称","型号","售价(可修改)","库存数量"};
		data=cdata;
...
//		check.add(showpromotion);
	...
	//销售退货单
	public SalesP2(Object[][] cdata,AddSalesFrame asf,SalesVO svo){
		this.setLayout(new BorderLayout());
		addSalesframe=asf;
		salessvo=svo;
		...
		String[] columnTitle={"退货数量","编号","名称","型号","售价(可修改)"};
		data=cdata;
		commoditytable = new JTable(new TableModel(data, columnTitle){
			public boolean isCellEditable(int row, int column) {
				return (column == 4);
	...
	private class nextListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			if(customerbox.getSelectedItem().equals("空")||getremakes.getText().isEmpty()){
				...
				if((Double.parseDouble(getdiscount.getText())<=4000&&addSalesframe.userlevel.equals("经理"))||(Double.parseDouble(getdiscount.getText())<=1000&&addSalesframe.userlevel.equals("普通职员"))){
					try{
						...
						addSalesframe.panel3=new SalesP3(newdata,svo,addSalesframe);
