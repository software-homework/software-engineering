...
public class AddTotalPromDialog extends AddPromotionDialog{
	private PromotionBLService bl;
	private GetCommodity get;
	private TotalPromPane pane;
	//------------------------------------------------
	public AddTotalPromDialog(TotalPromPane pane) {
		super();
		this.pane = pane;
		// 设置填充面板
		pane2 = new JPanel();
		pane2.setLayout(new GridLayout(5, 2, 50, 50));
		pane2.setBorder(BorderFactory.createEmptyBorder(120, 0, 50, 70));
		
		label1 = new JLabel("起始日期(yyyy/MM/dd)：", JLabel.RIGHT);
		label2 = new JLabel("结束日期(yyyy/MM/dd)：", JLabel.RIGHT);
		label3 = new JLabel("总价：", JLabel.RIGHT);
		label4 = new JLabel("代金券：", JLabel.RIGHT);
		...
		label5 = new JLabel("赠品：", JLabel.LEFT);
		...
		public void actionPerformed(ActionEvent arg0) {
			if(isDateRight(text1.getText(), text2.getText())) {
				if(isNumberRight(text3.getText()) && isNumberRight(text4.getText())) {
					...
					bl = new PromotionBL();
					if(bl.addTotalProm(new TotalPromVO(text1.getText(),
													   text2.getText(),
						...
						logBLService.addLog(new LogVO(pane.frame.getUserName(), "添加针对总价的促销策略"));
					}else {
