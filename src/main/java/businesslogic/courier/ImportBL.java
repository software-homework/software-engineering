...
import data.courier.ImportData;
...
public class ImportBL implements ImportBLService,businesslogic.logistics.GetImport,businesslogicservice.manager.GetImport,GetImportBL{
	private ImportDataService data = null;
	SetMessage sm=new MessageBL();
	GetCommodity gc=new CommodityBL();
	public ImportBL(){
		try {
			data = ImportData.GetInstance();
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
