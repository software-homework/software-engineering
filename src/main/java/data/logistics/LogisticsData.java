...
public class LogisticsData extends UnicastRemoteObject implements LogisticsDataService {
	...
	private static LogisticsData sd;

	private LogisticsData() throws RemoteException {
		root = new CategoryPO();
		root.setId("-1");
		root.setName("�ܷ���");
		readFromFile();
	...
	public static LogisticsData getInstance() throws RemoteException {
		if (sd == null) {
			sd = new LogisticsData();
		}
		return sd;
	}
	...
	public boolean add(CommodityPO commodity, CategoryPO father) {
		...
		for (CommodityPO c : showStock(root)) {
			if (c.getCommodityName().equals(commodity.getCommodityName())
					&& c.getModel().equals(commodity.getModel())) {
				return false;
			}
		}
	...
	public boolean update(CommodityPO commodity, CommodityPO newCommodity, CategoryPO father) {
		...
		/*
		 * for(CommodityPO c : showStock(root)){
		 * if(c.getCommodityName().equals(newCommodity.getCommodityName()) &&
		 * c.getModel().equals(newCommodity.getModel())){ return false; } }
		 */
