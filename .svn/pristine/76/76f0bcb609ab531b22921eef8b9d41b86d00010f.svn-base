...
public class ChangePassword extends JDialog{
		...
		public void actionPerformed(ActionEvent e) {
			if (!String.valueOf(password2.getPassword()).trim().equals("")) {
				if (String.valueOf(password2.getPassword()).equals(String.valueOf(password3.getPassword()))) {
					...
					list = bl.findUser();
					for (UserVO i : list) {
						if (i.name.equals(oldVO.name)
							&& i.type.equals(oldVO.type)
							&& i.permission.equals(oldVO.permission)) {
							if (String.valueOf(password1.getPassword()).equals(i.password)) {
								if (bl.updateUser(oldVO, new UserVO(i.type, i.name, String.valueOf(password2.getPassword()), i.permission))) {
									LogBLService logBLService = new LogBL();
									logBLService.addLog(new LogVO(oldVO.name, "修改用户密码"));
									JOptionPane.showMessageDialog(null, "修改用户密码成功！", "信息", JOptionPane.INFORMATION_MESSAGE);
								} else {
	...
	private class CancelListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			if (JOptionPane.showConfirmDialog(null, "确认要取消修改吗？", "提示", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE) == JOptionPane.YES_OPTION) {
				dispose();
			}
		}
	}
